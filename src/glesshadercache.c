...
void init_original_functions() {
    // TODO: can we somehow avoid this dlopen?
    void *realGLESv2 = dlopen("libGLESv2.so.2", RTLD_NOW);
    if (!realGLESv2) {
        debug("Bad news: couldn't open libGLESv2.so!\n");
        exit(EXIT_FAILURE);
    }

    real_glCompileShader = (PFNGLCOMPILESHADERPROC)dlsym(realGLESv2, "glCompileShader");
    real_glLinkProgram = (PFNGLLINKPROGRAMPROC)dlsym(realGLESv2, "glLinkProgram");
    real_glGetShaderiv = (PFNGLGETSHADERIVPROC)dlsym(realGLESv2, "glGetShaderiv");
